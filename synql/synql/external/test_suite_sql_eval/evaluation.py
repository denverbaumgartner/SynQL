...
def build_scores_dict(scores, etype, include_turn_acc=True):
    turns = ['turn 1', 'turn 2', 'turn 3', 'turn 4', 'turn > 4']
    levels = ['easy', 'medium', 'hard', 'extra', 'all']
    ...
    for level in levels:
        result_dict[level] = {}
        result_dict[level]['count'] = scores[level]['count']
        
        if etype in ["all", "exec"]:
            result_dict[level]['execution_accuracy'] = scores[level]['exec']

        if etype in ["all", "match"]:
            result_dict[level]['exact_matching_accuracy'] = scores[level]['exact']

            # Partial matching accuracy
            result_dict[level]['partial_matching_accuracy'] = {}
            for type_ in partial_types:
                result_dict[level]['partial_matching_accuracy'][type_] = scores[level]['partial'][type_]['acc']

            # Partial matching recall
            result_dict[level]['partial_matching_recall'] = {}
            for type_ in partial_types:
                result_dict[level]['partial_matching_recall'][type_] = scores[level]['partial'][type_]['rec']

            # Partial matching f1
            result_dict[level]['partial_matching_f1'] = {}
            for type_ in partial_types:
                result_dict[level]['partial_matching_f1'][type_] = scores[level]['partial'][type_]['f1']

    # Turns data
    if include_turn_acc:
        result_dict['turns'] = {}
        for turn in turns:
            result_dict['turns'][turn] = {}
            result_dict['turns'][turn]['count'] = scores[turn]['count']

            if etype in ["all", "exec"]:
                result_dict['turns'][turn]['execution_accuracy'] = scores[turn]['exec']

            if etype in ["all", "match"]:
                result_dict['turns'][turn]['exact_matching_accuracy'] = scores[turn]['exact']
...
async def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    correctness_list = []  # 1 for correct, 0 for incorrect
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            exec_correct = 0 # 1 for correct, 0 for incorrect
            p, g = pg
            p_str = p[0]
            ...
            if etype in ["all", "exec"]:
                ...
                if exec_score:
                    ...
                    exec_correct = 1 # 1 for correct, 0 for incorrect
                else:
                    turn_scores['exec'].append(0)
            correctness_list.append(exec_correct)
    ...
    return build_scores_dict(scores, etype, include_turn_acc=include_turn_acc), correctness_list
